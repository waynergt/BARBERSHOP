...
export default function AdminDashboard() {
  ...
  const citasFiltradas = busqueda 
    ? citas.filter(c => c.telefono?.includes(busqueda) || c.clienteNombre.toLowerCase().includes(busqueda.toLowerCase()))
    : citas;
      ...
      {fechas.map((fecha) => {
        ...
        return (
          <div key={fecha} className="relative group mb-4">
            <button onClick={() => toggleFecha(fecha)} className={`sticky top-0 z-10 w-full backdrop-blur py-3 mb-1 border-b flex items-center justify-between transition-all px-3 rounded-lg cursor-pointer ${isExpanded ? 'bg-zinc-900 border-zinc-700' : 'bg-zinc-950/80 border-zinc-800 hover:bg-zinc-900/50'}`}>
              <div className="flex items-center gap-3">
                <div className={`w-2 h-2 rounded-full transition-colors ${isExpanded ? 'bg-red-500' : 'bg-zinc-700'}`}></div>
                <h2 className={`text-lg font-bold capitalize ${isExpanded ? 'text-white' : 'text-zinc-400'}`}>{formatearFecha(fecha)}</h2>
                <span className="text-xs text-zinc-500 bg-zinc-900 px-2 py-0.5 rounded-full border border-zinc-800">{citasPorFecha[fecha].length}</span>
              </div>
            ...
            <div className={`grid gap-3 pl-4 border-l-2 border-zinc-900 transition-all duration-500 overflow-hidden ${isExpanded ? 'max-h-[50000px] opacity-100 py-2' : 'max-h-0 opacity-0 py-0'}`}>
              {citasPorFecha[fecha].map((cita) => {
                const isCancelled = cita.estado === 'cancelada';
                
                // Mensaje automático para cancelar
                const mensajeAviso = encodeURIComponent(`Hola ${cita.clienteNombre}, te saludamos de JBarber Shop. Por motivos de fuerza mayor (salud), lamentablemente tenemos que suspender las citas de estos días. Nos comunicaremos contigo cuando retomemos labores para reprogramar. ¡Mil disculpas por el inconveniente!`);
                const urlWaAviso = `https://wa.me/502${cita.telefono}?text=${mensajeAviso}`;
                  ...
                  <div key={cita.id} className={`relative p-4 rounded-lg flex justify-between items-center transition-all ${isCancelled ? 'bg-zinc-900/30 border border-transparent opacity-50' : 'bg-zinc-900 border border-zinc-800 hover:border-zinc-600 shadow-lg'}`}>
                        ...
                        <div className="flex flex-col gap-1 mt-1">
                           ...
                           {isCancelled && (
                             <div className="flex items-center gap-2 mt-1">
                                ...
                                {cita.motivoCancelacion && <span className="text-[10px] text-zinc-500 italic max-w-[200px] truncate">"{cita.motivoCancelacion}"</span>}
                             </div>
                           )}
                        </div>
                      </div>
                    ...
                    {!isCancelled && (
                      <div className="flex gap-2">
                        {/* NUEVO BOTÓN: Enviar WhatsApp de disculpas */}
                        <a href={urlWaAviso} target="_blank" rel="noopener noreferrer" className="text-zinc-500 hover:text-green-500 p-2 hover:bg-zinc-800 rounded-full transition-colors" title="Avisar por WhatsApp">
    ...
    <div className="min-h-screen bg-zinc-950 p-4 md:p-8 font-sans text-gray-200 pb-20">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <button onClick={cargarCitas} className="bg-zinc-800 hover:bg-zinc-700 text-xs uppercase font-bold py-2 px-4 rounded transition-colors border border-zinc-700">↻ Actualizar</button>
        </div>
        ...
        <div className="mb-6 relative">
          ...
          <input type="text" value={busqueda} onChange={(e) => { setBusqueda(e.target.value); if(e.target.value) { const todasAbiertas = Object.keys(citasPorFecha).reduce((acc, fecha) => ({...acc, [fecha]: true}), {}); setFechasExpandidas(todasAbiertas); setMostrarHistorial(true); } else { setFechasExpandidas({}); setMostrarHistorial(false); } }} placeholder="Buscar por celular o nombre..." className="w-full bg-zinc-900 border border-zinc-800 text-white text-sm rounded-xl pl-10 pr-4 py-3 focus:border-red-600 focus:ring-1 focus:ring-red-600 outline-none transition-all placeholder:text-zinc-600" />
        </div>

        <div className="grid grid-cols-3 gap-4 mb-8">
          {!busqueda ? (
            <>
              <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 text-center"><span className="text-2xl font-bold text-white block">{totalHoy}</span><span className="text-xs text-zinc-500 uppercase tracking-wider">Citas Hoy</span></div>
              <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 text-center"><span className="text-2xl font-bold text-emerald-400 block">{totalPendientes}</span><span className="text-xs text-zinc-500 uppercase tracking-wider">Activas Global</span></div>
              <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 text-center"><span className="text-2xl font-bold text-red-400 block">{totalCanceladas}</span><span className="text-xs text-zinc-500 uppercase tracking-wider">Canceladas Global</span></div>
            </>
          ) : (
            <>
              <div className="bg-blue-900/20 p-4 rounded-xl border border-blue-800/50 text-center relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div><span className="text-2xl font-bold text-blue-200 block">{busquedaTotal}</span><span className="text-xs text-blue-400 uppercase tracking-wider font-bold">Historial Total</span></div>
              <div className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 text-center"><span className="text-2xl font-bold text-white block">{busquedaEfectivas}</span><span className="text-xs text-zinc-500 uppercase tracking-wider">Asistidas</span></div>
              <div className="bg-red-900/20 p-4 rounded-xl border border-red-800/50 text-center relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-red-600"></div><span className="text-2xl font-bold text-red-400 block">{busquedaCanceladas}</span><span className="text-xs text-red-400 uppercase tracking-wider font-bold">Canceladas</span></div>
            </>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {todasLasFechas.length === 0 && (
              <div className="text-center py-10 bg-zinc-900/50 rounded-xl border border-zinc-800 border-dashed"><p className="text-zinc-500">{busqueda ? `No se encontraron citas para "${busqueda}"` : "No hay citas registradas aún."}</p></div>
            )}
            ...
            {!busqueda && fechasPasadas.length > 0 && (
              <div className="mt-12 pt-8 border-t border-zinc-900">
                <button onClick={() => setMostrarHistorial(!mostrarHistorial)} className="flex items-center gap-2 text-zinc-500 hover:text-zinc-300 transition-colors mx-auto text-sm font-medium">
                ...
                <div className={`transition-all duration-500 overflow-hidden ${mostrarHistorial ? 'max-h-[50000px] opacity-100 mt-6' : 'max-h-0 opacity-0 mt-0'}`}>
                   <RenderGrupoFechas fechas={fechasPasadas} />
                </div>
              </div>
            )}
            {busqueda && fechasPasadas.length > 0 && <RenderGrupoFechas fechas={fechasPasadas} />}
          </div>
        )}
      </div>
    </div>
